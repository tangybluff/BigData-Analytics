--Retrieve the unique cities and their respective countries 
--from the 'air_pollution_pm' table.
...
SELECT Year, sum("Air_pollution_(total)_(deaths_per_100,000)") AS "Total Deaths"
FROM deathratesfromairpollution as d
group by 1
order by d."Air_pollution_(total)_(deaths_per_100,000)" desc;

--Calculate the average PM2.5 levels ('PM2.5_(μg/m3)') 
--for each city  between specific years from the 'air_pollution_pm' table. 
SELECT City, AVG("PM2.5_(μg/m3)") AS "Average PM2.5 Level"
FROM AirPollution
WHERE Year = 2011 and 2020  -- Replace with the desired year
GROUP BY City;

--Retrieve the top 10 countries 
-- with the highest manufacturing output 
-- in the year 2000 from the 'global_top_manufact' table.
SELECT Country, "2000" AS "Manufacturing Output (2000)"
FROM WorldTop10Manufacturers19762020
ORDER BY "Manufacturing Output (2000)" DESC
LIMIT 10;

--Find the country with the highest percentage of the population 
--living below $1.90 a day ('below_1.90$a_day(World_Bank_(2019))') in the year 2015 from the 'poverty_population_distribution' table.
SELECT Entity, 
"below_1.90$_a_day_(World_Bank_(2019))" AS "Percentage Below $1.90 (2015)"
FROM distributionofpopulationpovertythresholds
WHERE Year = 2015 AND Entity IS NOT 'World'
ORDER BY "Percentage Below $1.90 (2015)" DESC
LIMIT 1;

--Calculate the average PM10 levels ('PM10_(μg/m3)') 
-- for each country and year from the 'air_pollution_pm' table. 
-- Include only data from 1990 to 2015:
...
INNER JOIN deathratesfromairpollution dr 
ON ap.Country_Name = dr.Entity 
AND ap.Year = dr.Year;
/******************************************************************************/
with tablaBien as (SELECT ap.Country_Name,
       					   ap.Year,
                           avg(ap."PM2.5_(μg/m3)") as mediapm25
                    FROM AirPollution ap
                    group by 1,2)

select t1.country_name, t1.Year, t1.mediapm25,
	   dr."Air_pollution_(total)_(deaths_per_100,000)"
from tablaBien t1
INNER JOIN deathratesfromairpollution dr 
ON t1.Country_Name = dr.Entity 
AND t1.Year = dr.Year

where mediapm25 not null 
and t1.year = 2017;
/******************************************************************************/
